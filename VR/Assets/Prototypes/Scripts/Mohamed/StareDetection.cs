...
public class StareDetection : MonoBehaviour {
	...
	void Update () {
		...
		if (Physics.Raycast(transform.position,transform.forward,out hit, 10.0f)) {
			...
			if (timer < coolDown) interactibleObjects[i].GetComponent<InteractibleObject>().selected = false;}//remove the "if" 2 b able to stop mid travel
	...
	public void GoToIO (Vector3 Destination) {
		transform.parent.transform.position = Vector3.MoveTowards(transform.parent.transform.position,Destination, 3*Time.deltaTime);
